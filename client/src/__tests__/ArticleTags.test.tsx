...
describe("Article Tags", () => {
	...
	test("clicking tags should filter correctly", async () => {
		...
		mock
			.onGet(`/api/comment/${fakeArticles[2]._id}`)
			.reply(200, { comments: [] });
		...
		const expressTag = await screen.findByRole("button", { name: /express/i });
